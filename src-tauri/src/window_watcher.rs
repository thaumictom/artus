            ...
            if focused {
                ...
                if let Some(overlay) = app_handle.get_webview_window("overlay") {
                    if !use_window_ownership && hide_on_focus_loss && !cleanup_on_focus_loss {
                        ...
                        if was_visible {
                            info!("Restoring overlay visibility after focus gain");
                            let _ = overlay.show();
                            app_handle
                                .state::<AppState>()
                                .overlay_was_visible
                ...
                if !is_relic_mode && !use_window_ownership {
                    if hide_on_focus_loss {
                        if cleanup_on_focus_loss {
                            info!("Hiding & cleaning up overlay due to focus loss (manual mode)");
                            let _ = crate::ocr::hide_overlay(&app_handle);
                        } else {
                            info!("Visually hiding overlay due to focus loss (manual mode)");
                            if let Some(overlay) = app_handle.get_webview_window("overlay") {
                    ...
                    info!("Keeping overlay as is (relic rewards mode or sandwich mode)");
                }
            }
        }
    });
